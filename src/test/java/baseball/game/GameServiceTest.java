...
class GameServiceTest {
    ...
    void compareGameNumber(String randomNumber, String inputNumber) {
        ...
        for (int i = 0; i < randomNumber.length(); i++) {
            randomList.add((int) randomNumber.charAt(i));
        }
        for (int i = 0; i < inputNumber.length(); i++) {
            inputList.add((int) inputNumber.charAt(i));
        }
        ...
        // when
        for (int i = 0; i < randomNumber.length(); i++) {
            strike += compareTwoNumber(randomList.get(i), inputList.get(i));
        }

        Set<Integer> randomSet = new HashSet<>(randomList);
        for (Integer i : inputList) {
            ball += containSetNumber(randomSet, i);
        }
        ball -= strike;
